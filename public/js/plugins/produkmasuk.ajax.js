...
class RowsAjax {
  ...
  _createInstance() {
    ...
    this._datatable = jQuery('#datatableRowsAjax').DataTable({
      ...
      info: false,
      ajax: 'http://127.0.0.1:8000/ProdukMasuk',
      ...
      pageLength: 10,
      columns: [{ data: 'created_at' }, { data: 'produk.SKU_produk' }, { data: 'produk.nama' }, { data: 'jumlah' }, { data: 'produk.vp' }],
      language: {
        paginate: {
      ...
      columnDefs: [
        ...
        //   targets: 1,
        //   render: function (data, type, row, meta) {
        //     return '<a class="list-item-heading body" href="#">' + 'ayam' + '</a>';
        //   },
        // },
  ...
  _initJson() {
    if (document.getElementById('SKU_produk') !== null) {
      new AutocompleteCustom('SKU_produk', 'SKU_produkResults', {
        data: {
          src: async () => {
            const source = await fetch('/ProdukMasuk/create');
        ...
        onSelection: (feedback) => {
          document.getElementById('SKU_produk').value = feedback.selection.value['SKU_produk'];
          document.getElementById('nama_produk').value = feedback.selection.value['nama'];
          document.getElementById('produk_id').value = feedback.selection.value['id_produk'];
          document.getElementById('SKU_produk').blur();
        },
      });
    }
  }
  ...
  _setForm() {
    const data = this._rowToEdit.data();
    document.querySelector('#addEditModal input[name=Nama]').value = data.produk.nama;
    document.querySelector('#addEditModal input[name=Stock]').value = data.stock;
    document.querySelector('#addEditModal input[name=VP]').value = data.produk.vp;
    document.querySelector('#addEditModal input[name=Harga_p]').value = data.produk.harga_produk;
    document.querySelector('#addEditModal input[name=SKU_p]').value = data.produk.SKU_produk;
    document.querySelector('#addEditModal textarea[name=Deskripsi_p]').value = data.produk.deskripsi;
  }
  ...
  _getFormData() {
    ...
    data.produk_id = document.querySelector('#addEditModal input[name=produk_id]').value;
